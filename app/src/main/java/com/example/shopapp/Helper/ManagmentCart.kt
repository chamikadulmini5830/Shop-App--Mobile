...
class SliderAdapter(
    ...
    override fun onBindViewHolder(holder: SliderAdapter.SliderViewHolder, position: Int) {
        holder.setImage(sliderItems[position % sliderItems.size], context) // Handle looping

        // Logic to handle infinite scrolling
        if (position == sliderItems.size - 1) {
            viewPager2.post {
                viewPager2.setCurrentItem(0, false) // Snap to the first item without animation
            }
        }
    }

    override fun getItemCount(): Int = Integer.MAX_VALUE // To simulate infinite scrolling
}
